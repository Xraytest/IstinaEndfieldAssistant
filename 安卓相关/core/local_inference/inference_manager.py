...
if __name__ == "__main__":
    ...
    print(f"  满足要求: {gpu_info.get('meets_requirements', False)}")
    print(f"  GPU数量: {gpu_info.get('gpu_count', 0)}")
    
    # 测试模式
    print("\n2. 推理模式:")
    print(f"  当前模式: {manager.get_current_mode()}")
    print(f"  可用模式: {manager.get_available_modes()}")
    print(f"  本地可用: {manager.is_local_available()}")
    print(f"  异步可用: {manager.is_async_available()}")
    ...
    print(f"  首次运行: {stats['first_run']}")
    print(f"  异步任务数: {stats['async_tasks_count']}")
