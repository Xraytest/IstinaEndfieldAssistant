"""
设备状态管理器 - 负责检测和恢复设备状态，确保任务能在任何页面正确执行
"""
import time
import logging
...
class DeviceStateManager:
    ...
    def __init__(self, screen_capture, touch_executor, communicator, auth_manager):
        ...
        self.state_templates = {
            'game_main': ['Resell/inGame1.png', 'Resell/inGame2.png'],
            'friend_list': ['VisitFriends/OnFriendList.png'],
            'login_confirm': ['SceneManager/LoginConfirm.png'],
            'terminal': ['SceneManager/Terminal.png'],
            'home_screen': ['SceneManager/HomeScreen.png'],
            'error_dialog': ['SceneManager/ErrorDialog.png'],
            'loading_screen': ['SceneManager/LoadingIcon.png']
        }
        
        # 状态恢复策略
        self.recovery_strategies = {
            'unknown': self._recover_from_unknown_state,
            'error_dialog': self._recover_from_error_dialog,
            'loading_screen': self._recover_from_loading_screen,
    ...
    def detect_current_state(self, device_serial: str) -> str:
        ...
        try:
            # 截取屏幕
            screen_data = self.screen_capture.capture_screen(device_serial)
            if not screen_data:
                self.logger.error(LogCategory.DEVICE, "屏幕捕获失败，无法检测状态")
                return 'unknown'
            ...
            request_data = {
                ...
                "current_task": "state_detection",
                "task_variables": {},
                "device_info": self.screen_capture.get_device_info(device_serial)
            }
            
            # 发送状态检测请求
            response = self.communicator.send_request("detect_device_state", request_data)
            if response and response.get('status') == 'success':
                detected_state = response.get('data', {}).get('detected_state', 'unknown')
                confidence = response.get('data', {}).get('confidence', 0.0)
                
                self.logger.debug(LogCategory.DEVICE, f"设备状态检测结果: {detected_state} (置信度: {confidence:.2f})")
                return detected_state
            
            # 如果服务器检测失败，使用本地模板匹配
            return self._detect_state_with_templates(screen_data, device_serial)
            
        except Exception as e:
            self.logger.exception(LogCategory.DEVICE, f"设备状态检测异常: {e}")
            return 'unknown'
    
    def _detect_state_with_templates(self, screen_data: bytes, device_serial: str) -> str:
        """使用本地模板匹配检测状态"""
        try:
            ...
            # 将屏幕数据转换为OpenCV图像
            image_stream = io.BytesIO(screen_data)
            ...
            for state_name, template_paths in self.state_templates.items():
                for template_path in template_paths:
                    try:
                        ...
                        if max_val > best_score and max_val > 0.7:  # 置信度阈值
                            best_score = max_val
    ...
    def _click_position(self, device_serial: str, x_ratio: float, y_ratio: float) -> bool:
        ...
        try:
            if self.touch_executor:
                return self.touch_executor.execute_tool_call(
                    device_serial, 
                    'click', 
                    {'coordinates': [x_ratio, y_ratio]}
                )
            return False
        except Exception as e:
