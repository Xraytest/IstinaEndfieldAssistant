...
class MaaFwTouchConfig:
    ...
    def __init__(self, press_duration_ms: int=50, press_jitter_px: int=2, swipe_delay_min_ms: int=100, swipe_delay_max_ms: int=300, use_normalized_coords: bool=True, fail_on_error: bool=True):
...
class MaaFwTouchExecutor:
    ...
    def __init__(self, adb_manager, config: Optional[MaaFwTouchConfig]=None):
        ...
        self.logger.info(LogCategory.MAIN, 'MaaFramework 触控执行器初始化完成', normalized_coords=self.config.use_normalized_coords)
        ...
        if controller:
            try:
                ...
                if image is not None:
                    ...
                    self.logger.debug(LogCategory.MAIN, f'通过 MaaFramework 获取分辨率：{width}x{height}')
                    return resolution
            except Exception as e:
                self.logger.debug(LogCategory.MAIN, f'通过 MaaFramework 获取分辨率失败：{e}')
        if hasattr(self.adb_manager, '_run_adb_command'):
            success, output = self.adb_manager._run_adb_command(['-s', device_serial, 'shell', 'wm', 'size'])
            if success and 'Physical size:' in output:
                try:
                    size_str = output.split(':')[-1].strip()
                    width, height = map(int, size_str.split('x'))
                    self.cached_resolution[device_serial] = (width, height)
                    self.logger.debug(LogCategory.MAIN, f'通过 wm size 获取分辨率：{width}x{height}')
                    return (width, height)
                except Exception as e:
                    self.logger.debug(LogCategory.MAIN, f'解析 wm size 输出失败：{e}')
        return (1080, 1920)
    ...
    def _get_or_create_controller(self, device_serial: str) -> Optional[AdbController]:
        ...
        try:
            ...
            if not target_device:
                self.logger.warning(LogCategory.MAIN, f'未找到设备 {device_serial}，尝试直接连接')
                from maa.define import MaaAdbScreencapMethod, MaaAdbInputMethod
                target_device = type('MockDevice', (), {'adb_path': getattr(self.adb_manager, 'adb_path', 'adb'), 'address': device_serial, 'screencap_methods': MaaAdbScreencapMethod.All, 'input_methods': MaaAdbInputMethod.All, 'config': ''})()
            controller = AdbController(adb_path=target_device.adb_path, address=target_device.address, screencap_methods=target_device.screencap_methods, input_methods=target_device.input_methods, config=target_device.config)
            connection_result = controller.post_connection().wait()
            if not connection_result:
                self.logger.error(LogCategory.MAIN, f'MaaFramework 连接设备失败：{device_serial}')
                return None
            self.connected_devices[device_serial] = controller
            self.logger.info(LogCategory.MAIN, f'MaaFramework 设备连接成功：{device_serial}')
            return controller
        except Exception as e:
            self.logger.exception(LogCategory.MAIN, f'创建 MaaFramework 控制器失败：{e}')
            return None
    ...
    def _normalize_to_pixel_coords(self, device_serial: str, norm_x: float, norm_y: float) -> Tuple[int, int]:
        ...
        self.logger.debug(LogCategory.MAIN, f'归一化坐标转换：({norm_x:.3f},{norm_y:.3f}) → ({pixel_x},{pixel_y})')
    ...
    def _validate_pixel_coords(self, device_serial: str, x: int, y: int) -> Tuple[int, int]:
        ...
        if (pixel_x, pixel_y) != (x, y):
            self.logger.debug(LogCategory.MAIN, f'坐标修正：({x},{y}) → ({pixel_x},{pixel_y})')
        return (pixel_x, pixel_y)
    ...
    def clear_resolution_cache(self, device_serial: Optional[str]=None):
        if device_serial:
            if device_serial in self.cached_resolution:
                ...
                self.logger.debug(LogCategory.MAIN, f'清除设备分辨率缓存：{device_serial}')
        else:
    ...
    def safe_press(self, device_serial: str, x: int, y: int, purpose: str='点击') -> bool:
        ...
        if start_x == end_x and start_y == end_y:
            start_x = max(0, x - 1)
            start_y = y
        self.logger.info(LogCategory.MAIN, f'👆 MAA安全按压 ({start_x},{start_y})→({end_x},{end_y}) duration={self.config.press_duration_ms}ms | 抖动±{jitter} | purpose={purpose}')
        controller = self._get_or_create_controller(device_serial)
        if not controller:
        ...
        try:
            ...
            if result:
                self.logger.info(LogCategory.MAIN, f'MaaFramework 点击执行成功：({end_x},{end_y})')
            else:
                self.logger.error(LogCategory.MAIN, f'MaaFramework 点击执行失败：({end_x},{end_y})')
                return False
        except Exception as e:
            self.logger.exception(LogCategory.MAIN, f'MaaFramework 点击执行异常：{e}')
            return False
        delay = random.uniform(self.config.swipe_delay_min_ms, self.config.swipe_delay_max_ms)
        time.sleep(delay / 1000)
        self.logger.debug(LogCategory.MAIN, f'MAA延迟: {delay:.1f}ms')
        return True

    def safe_swipe(self, device_serial: str, x1: int, y1: int, x2: int, y2: int, duration: int=300, purpose: str='滑动') -> bool:
        jitter = self.config.press_jitter_px
        ...
        self.logger.info(LogCategory.MAIN, f'👆 MAA安全滑动 ({start_x},{start_y})→({end_x},{end_y}) duration={duration}ms | 抖动±{jitter} | purpose={purpose}')
        controller = self._get_or_create_controller(device_serial)
        if not controller:
            self.logger.error(LogCategory.MAIN, '无法获取 MaaFramework 控制器')
            return False
        try:
            ...
            if result:
                self.logger.info(LogCategory.MAIN, f'MaaFramework 滑动执行成功：({start_x},{start_y})→({end_x},{end_y})')
            else:
                self.logger.error(LogCategory.MAIN, f'MaaFramework 滑动执行失败：({start_x},{start_y})→({end_x},{end_y})')
                return False
        except Exception as e:
            self.logger.exception(LogCategory.MAIN, f'MaaFramework 滑动执行异常：{e}')
            return False
        delay = random.uniform(self.config.swipe_delay_min_ms, self.config.swipe_delay_max_ms)
        time.sleep(delay / 1000)
        self.logger.debug(LogCategory.MAIN, f'MAA延迟: {delay:.1f}ms')
    ...
    def safe_long_press(self, device_serial: str, x: int, y: int, duration_ms: int=500, purpose: str='长按') -> bool:
        jitter = self.config.press_jitter_px
        jitter_x = random.randint(-jitter, jitter)
        jitter_y = random.randint(-jitter, jitter)
        start_x = x + jitter_x
        ...
        end_x = x
        end_y = y
        self.logger.info(LogCategory.MAIN, f'👆 MAA安全长按 ({start_x},{start_y})→({end_x},{end_y}) duration={duration_ms}ms | 抖动±{jitter} | purpose={purpose}')
        controller = self._get_or_create_controller(device_serial)
        if not controller:
            self.logger.error(LogCategory.MAIN, '无法获取 MaaFramework 控制器')
            return False
        try:
            ...
            if result:
                self.logger.info(LogCategory.MAIN, f'MaaFramework 长按执行成功：({start_x},{start_y})→({end_x},{end_y})')
            else:
                self.logger.error(LogCategory.MAIN, f'MaaFramework 长按执行失败：({start_x},{start_y})→({end_x},{end_y})')
                return False
        except Exception as e:
            self.logger.exception(LogCategory.MAIN, f'MaaFramework 长按执行异常：{e}')
            return False
        delay = random.uniform(self.config.swipe_delay_min_ms, self.config.swipe_delay_max_ms)
        time.sleep(delay / 1000)
        self.logger.debug(LogCategory.MAIN, f'MAA延迟: {delay:.1f}ms')
    ...
    def _input_text(self, device_serial: str, text: str) -> bool:
        ...
        try:
            ...
            if result:
                self.logger.debug(LogCategory.MAIN, f'文本输入执行成功：{len(text)}字符')
                return True
            else:
                self.logger.error(LogCategory.MAIN, f'文本输入执行失败：{text}')
                return False
        except Exception as e:
            self.logger.exception(LogCategory.MAIN, f'文本输入执行异常：{e}')
            return False
    ...
    def _press_key(self, device_serial: str, key_code: str) -> bool:
        ...
        if hasattr(self.adb_manager, '_run_adb_command'):
            ...
            if success:
                self.logger.debug(LogCategory.MAIN, f'按键操作执行成功：{key_code}')
            else:
                self.logger.exception(LogCategory.MAIN, f'按键操作执行失败：{key_code}')
            return success
        else:
    ...
    def _press_system_button(self, device_serial: str, button_name: str) -> bool:
        button_codes = {'back': 4, 'home': 3, 'recent': 187}
    ...
    def execute_tool_call(self, device_serial: str, action: str, params: Dict) -> bool:
        self.logger.info(LogCategory.MAIN, f'执行工具调用：{action}')
        action_mapping = {'click': 'safe_press', 'swipe': 'safe_swipe', 'long_press': 'safe_long_press', 'drag': 'safe_swipe', 'type_text': 'input_text', 'input': 'input_text', 'key': 'press_key'}
        ...
        if mapped_action == 'safe_press' or action == 'click':
            ...
            self.logger.debug(LogCategory.MAIN, f'等待完成：{duration_ms}ms')
            return True
            ...
            self.logger.warning(LogCategory.MAIN, f'未知工具类型：{action}')
            return False