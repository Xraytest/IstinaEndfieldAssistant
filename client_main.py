...
class ReAcrtureClientGUI:
    ...
    def __init__(self, root):
        ...
        self.logger.info(LogCategory.MAIN, '程序启动', working_dir=os.path.dirname(os.path.abspath(__file__)))
    ...
    def setup_styles(self):
        ...
        try:
            ...
            ctypes.windll.dwmapi.DwmSetWindowAttribute(hwnd, DWMWA_USE_IMMERSIVE_DARK_MODE, ctypes.byref(ctypes.c_int(0)), ctypes.sizeof(ctypes.c_int))
    ...
    def _load_config(self, config_file):
        ...
        if os.path.exists(config_path):
            ...
            return {'server': {'host': '127.0.0.1', 'port': 9999}, 'adb': {'path': '3rd-part/ADB/adb.exe', 'timeout': 10}, 'git': {'path': '3rd-part/Git/bin/git.exe'}, 'screen': {'quality': 80, 'max_size': 1024}, 'touch': {'touch_method': 'maatouch', 'maa_style': {'enabled': true, 'press_duration_ms': 50, 'press_jitter_px': 2, 'swipe_delay_min_ms': 100, 'swipe_delay_max_ms': 300, 'use_normalized_coords': true}, 'swipe_duration_ms': 300, 'long_press_duration_ms': 500, 'minitouch': {'enabled': false, 'binary_path': 'client/3rd-part/minitouch/armeabi-v7a/minitouch'}, 'maatouch': {'enabled': true, 'binary_path': 'client/3rd-part/minitouch/maatouch/minitouch'}}, 'security': {'enable_safe_press': true, 'enable_jitter': true}, 'communication': {'password': 'default_password'}}
    ...
    def init_components(self):
        ...
        try:
            ...
            self.adb_manager = ADBDeviceManager(adb_path=adb_path, timeout=self.config['adb']['timeout'])
            self.logger.debug(LogCategory.MAIN, '初始化屏幕捕获模块')
            self.screen_capture = ScreenCapture(adb_manager=self.adb_manager)
            ...
            maa_config = MaaFwTouchConfig(press_duration_ms=maa_style_config.get('press_duration_ms', 50), press_jitter_px=maa_style_config.get('press_jitter_px', 2), swipe_delay_min_ms=maa_style_config.get('swipe_delay_min_ms', 100), swipe_delay_max_ms=maa_style_config.get('swipe_delay_max_ms', 300), use_normalized_coords=maa_style_config.get('use_normalized_coords', True), fail_on_error=fail_on_error)
            self.logger.info(LogCategory.MAIN, '触控配置加载完成（MaaFramework方案）')
            self.touch_executor = MaaFwTouchExecutor(adb_manager=self.adb_manager, config=maa_config)
            self.logger.info(LogCategory.MAIN, '触控执行器初始化完成（MaaFramework方案）', press_duration_ms=maa_config.press_duration_ms, jitter_px=maa_config.press_jitter_px, normalized_coords=maa_config.use_normalized_coords)
            self.logger.debug(LogCategory.MAIN, '初始化任务管理模块')
            self.task_manager = TaskManager(config_dir=os.path.join(os.path.dirname(__file__), 'config'), data_dir=os.path.join(os.path.dirname(__file__), 'data'))
            self.logger.debug(LogCategory.MAIN, '初始化通信模块')
            self.communicator = ClientCommunicator(host=self.config['server']['host'], port=self.config['server']['port'], password=self.config.get('communication', {}).get('password', 'default_password'), timeout=300)
            ...
            self.execution_manager = ExecutionManager(self.device_manager, self.screen_capture, self.touch_executor, self.task_queue_manager, self.communicator, self.auth_manager, get_device_type_callback=None)
            self.logger.debug(LogCategory.MAIN, '初始化GUI管理器')
            self.gui_manager = MainGUIManager(self.root, self.auth_manager, self.device_manager, self.execution_manager, self.task_queue_manager, self.config, self.log_message, self)
    ...
    def check_login_status(self):
        ...
        if is_logged_in:
            ...
            auth_gui = AuthManagerGUI(self.root, self.auth_manager, on_login_success=self.on_login_success)
    ...
    def load_task_queue(self):
        ...
        if os.path.exists(task_queue_file):
            ...
            recommended_task_chain = [{'id': 'task_visit_friends', 'name': '访问好友', 'variables': {'优先偷菜': '是', '访问数量': '全部', '交换线索': '是'}}, {'id': 'task_dijiang_rewards', 'name': '帝江号奖励', 'variables': {'自动开始交换': '否', '无控制中枢生产助力': '否', '线索设置': '是', '线索发送数量': '3', '线索库存上限': '3', '培养目标': '任意', '自动提取种子': '否'}}, {'id': 'task_credit_shopping', 'name': '积分购物', 'variables': {'优先购买': '嵌晶玉|武库配额', '自动领取积分': '是', '折扣要求': '不限', '购买黑名单物品': '否', '黑名单': '', '保留积分': '否'}}, {'id': 'task_sell_product', 'name': '出售产品', 'variables': {'出售地区': '四号谷地', '保留数量': '0'}}, {'id': 'task_daily_rewards', 'name': '每日奖励领取', 'variables': {'领取邮件奖励': '是', '领取任务奖励': '是', '领取送货奖励': '是', '领取活动奖励': '是', '领取协议通行证奖励': '是'}}]
    ...
    def log_message(self, message, category='general', level='INFO'):
        category_map = {'system': LogCategory.MAIN, 'adb': LogCategory.ADB, 'communication': LogCategory.COMMUNICATION, 'execution': LogCategory.EXECUTION, 'auth': LogCategory.AUTHENTICATION, 'task': LogCategory.EXECUTION, 'version': LogCategory.MAIN, 'general': LogCategory.MAIN}
        level_map = {'DEBUG': LogLevel.DEBUG, 'INFO': LogLevel.INFO, 'WARNING': LogLevel.WARNING, 'ERROR': LogLevel.EXCEPTION, 'CRITICAL': LogLevel.CRITICAL}
