...
def verify_stop_hook():
    ...
    # 3. 验证 VLM 决策器
    vlm_file = PROJECT_ROOT / 'src' / 'core' / 'vlm_decider.py'
    with open(vlm_file, 'r', encoding='utf-8') as f:
        vlm_content = f.read()
    
    has_decide = 'def decide(self, img: np.ndarray, prompt: str)' in vlm_content
    
    print(f'\n[VLM 决策器]')
    print(f'  decide(img, prompt) 简化方法：{"✅" if has_decide else "❌"}')
