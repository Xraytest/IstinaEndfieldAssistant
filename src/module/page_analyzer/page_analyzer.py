...
from module.page_analyzer.recognition import RecognitionEngine
...
class HighPrecisionPageAnalyzerV2:
    ...
    def _check_exit_dialog(self, img, w, h):
        ...
        if ok_sift:
            return True, {
                "method": "CancelButton+ZNCC",
                "bbox": rs.get("bbox"),
                "center": rs.get("center"),
                "confidence": rs.get("score", 0)
            }

        return False, {}

    def _check_quest_panel(self, img, w, h):
        """任务面板：TaskIcon 掩膜 ZNCC
    ...
    def _check_world(self, img, w, h):
        """世界页面：WorldMenu 掩膜 ZNCC 匹配 或 绿色资源图标
    ...
    def _check_in_game(self, img, w, h):
        """检查是否在 Endfield 游戏内：通过绿色资源图标检测"""
        # 使用绿色资源图标检测（右上角）
        rx = max(0, w-500)
        ok_green, _ = self.engine.recognize(img, {
            "type": "ColorMatch",
            "roi": [rx, 0, min(500, w-rx), min(150, h)],
            "lower": [35, 80, 80],
            "upper": [85, 255, 200],
