...
class ExecutionManager:
    ...
    def __init__(self, device_manager, screen_capture, touch_executor, task_queue_manager, communicator, auth_manager, config=None):
        ...
        # ========== 新增：截图记录器 ==========
        from .screencap_recorder import ScreencapRecorder
        self._screencap_recorder = ScreencapRecorder()
        self._screencap_recording = False
        
        # 初始化 InferenceManager（如果可用）
    ...
    def start_execution(self, log_callback, update_ui_callback, preview_update_callback=None):
        ...
        if not self.is_pc_mode:
            ...
            if not self.touch_executor.connected:
        ...
        # 检查并启动截图记录
        self._maybe_start_screencap_recording()
    ...
    def stop_execution(self):
        ...
        self.client_running = False
        # 停止截图记录
        self._stop_screencap_recording()
    ...
    def __del__(self):
        ...
        self.shutdown_screencap_recorder()

    # ========== 截图记录器相关方法 ==========
    
    def _maybe_start_screencap_recording(self) -> None:
        """检查配置并启动截图记录（如果启用）"""
        dev_config = self.config.get('dev', {})
        if dev_config.get('record_screencap', False) and not self._screencap_recording:
            self._start_screencap_recording()
            self._screencap_recording = True
    
    def _start_screencap_recording(self) -> None:
        """启动截图记录"""
        if self.is_pc_mode:
            # PC 模式使用触控管理器截图
            self._screencap_recorder.start(self.touch_executor.screencap)
        else:
            # Android 模式使用 ScreenCapture
            def capture_callback():
                current_device = self.device_manager.get_current_device()
                if current_device and self.screen_capture:
                    # 返回 numpy 数组格式
                    import numpy as np
                    ...
                    screen_data = self.screen_capture.capture_screen(current_device)
                    if screen_data:
                        image = Image.open(io.BytesIO(base64.b64decode(screen_data)))
                        return np.array(image)
                return None
            self._screencap_recorder.start(capture_callback)
        self.logger.info("截图记录已启动")
    
    def _stop_screencap_recording(self) -> None:
        """停止截图记录"""
        if self._screencap_recording:
            self._screencap_recorder.stop()
            self._screencap_recording = False
            self.logger.info(f"截图记录已停止，共记录 {self._screencap_recorder.get_screencap_count()} 张截图")
    
    def get_screencap_count(self) -> int:
        """获取已记录的截图数量"""
        return self._screencap_recorder.get_screencap_count()
    
    def is_screencap_recording(self) -> bool:
        """是否正在记录截图"""
        return self._screencap_recorder.is_recording()
    
    def get_screencap_cache_dir(self) -> str:
        """获取截图缓存目录"""
        return self._screencap_recorder.get_cache_dir()
    
    def clear_screencap_cache(self) -> int:
        """清空截图缓存"""
        return self._screencap_recorder.clear_cache()
    
    def shutdown_screencap_recorder(self) -> None:
        """关闭截图记录器"""
        if self._screencap_recording:
            self._stop_screencap_recording()
