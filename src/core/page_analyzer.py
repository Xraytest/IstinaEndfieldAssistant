...
完全弃用颜色分布/像素计数。使用：
1. TemplateMatch — 模板匹配定位特定 UI 元素
2. ColorMatch (轮廓) — 检测特定颜色的连通区域作为 UI 元素
3. And/Or 组合 — 多条件融合
...
class HighPrecisionPageAnalyzerV2:
    ...
    def _check_exit_dialog(self, img, w, h):
        ...
        # 必须同时满足：1) 对话框区域有金色元素  2) CancelButton模板匹配
        ...
        if ok_color and ok_sift:
            return True, {"method": "CancelButton+SIFT+Color", "pos": rs.get("location"),
                          "contours": rc.get("contours", 0)}

        if ok_sift:
            return True, {"method": "CancelButton+SIFT", "pos": rs.get("location")}
    ...
    def _check_quest_panel(self, img, w, h):
        ...
        if ok:
            return True, {"method": "TaskIcon", "pos": r.get("location")}
        # 回退：金色轮廓
        ok_color, rc = self.engine.recognize(img, {
            "type": "ColorMatch",
        ...
        if ok_color:
            return True, {"method": "QuestGoldContours", "contours": rc.get("contours")}
        return False, {}

    def _check_world(self, img, w, h):
        """世界页面：WorldMenu SIFT匹配 或 绿色资源图标"""
        ok, r = self.engine.recognize(img, {
            "type": "TemplateMatch",
        ...
        if ok:
            return True, {"method": "WorldMenu", "pos": r.get("location")}
        ...
        if ok_green:
            return True, {"method": "GreenResourceIcon", "contours": rc.get("contours")}
    ...
    def _check_menu(self, img, w, h):
        ...
        if ok:
            return True, {"method": "MenuGoldContours", "contours": rc.get("contours")}
        return False, {}
