...
用法（通过 istina.py）:
  ...
  istina.py scene explore [--depth 3]             # UI 探索
...
def cmd_analyze(args) -> int:
    ...
    opts = VLMOptions(
        ...
        system_prompt=args.system_prompt or "你是终末地UI分析器。输出JSON格式。",
    )
...
def cmd_explore(args) -> int:
    """启动 UI 探索（调用 explore_game.py）"""
    import subprocess
    script = os.path.join(PROJECT_ROOT, "scripts", "explore_game.py")
    if not os.path.exists(script):
        print(f"脚本未找到: {script}")
        return 1
    ...
    print(f"[explore] 启动探索 (depth={args.depth or 3})")
    print(f"[explore] 命令: {' '.join(cmd)}")
    result = subprocess.run(cmd, timeout=args.timeout or 1800)
