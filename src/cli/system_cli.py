...
def _check_server():
    ...
    try:
        ...
        return {"alive": r is not None, "response": bool(r)}
    except Exception as e:
...
def cmd_doctor(args) -> int:
    ...
    if os.path.exists(cfg_path):
        with open(cfg_path) as f:
            cfg = json.load(f)
        print(f"  服务器: {cfg.get('server', {}).get('host')}:{cfg.get('server', {}).get('port')}")
        print(f"  推理模式: {cfg.get('inference', {}).get('mode', 'N/A')}")
    else:
        print(f"  未找到配置文件")
