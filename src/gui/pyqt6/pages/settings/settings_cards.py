...
try:
    ...
    from core.core.local_inference.model_manager import ModelManager, ModelInfo
except ImportError:
    import sys
    import os
    ...
    from gui.pyqt6.theme.animation_manager import AnimationManager, fade_in_widget
    from gui.pyqt6.widgets.base_widgets import (
        PrimaryButton, SecondaryButton, CardWidget, ElevatedCardWidget, OutlinedCardWidget
    )
    from core.local_inference.model_manager import ModelManager, ModelInfo
...
class DevSettingsCard(CardWidget):
    """开发选项设置卡片"""
    ...
    def __init__(
        ...
        self._config = config or {}
        super().__init__(title="[工具] 开发选项", parent=parent)
        self._theme = ThemeManager.get_instance()
        self._setup_content()
    ...
    def _setup_content(self) -> None:
        ...
        # 记录运行截图开关
        screencap_frame = QFrame()
        screencap_layout = QHBoxLayout(screencap_frame)
        screencap_layout.setContentsMargins(0, 0, 0, 0)
        
        screencap_label = QLabel("记录运行截图:")
        screencap_label.setProperty("variant", "secondary")
        screencap_label.setFixedWidth(120)
        screencap_layout.addWidget(screencap_label)
        
        self._screencap_switch = AnimatedSwitch("启用截图记录")
        # 从配置中读取截图记录设置
        dev_config = self._config.get('dev', {})
        self._screencap_switch.setChecked(dev_config.get('record_screencap', False))
        self._screencap_switch.stateChanged.connect(self._on_screencap_changed)
        screencap_layout.addWidget(self._screencap_switch)
        
        screencap_tip = QLabel("启用后每秒缓存一张被控设备图像到 cache/screencap 目录")
        screencap_tip.setProperty("variant", "muted")
        screencap_tip.setWordWrap(True)
        screencap_layout.addWidget(screencap_tip, 1)
        
        layout.addWidget(screencap_frame)
        ...
        self._screencap_path_display = QLabel("cache/screencap/")
        self._screencap_path_display.setProperty("variant", "muted")
        path_layout.addWidget(self._screencap_path_display)
        ...
        stats_label = QLabel("已记录:")
        stats_label.setProperty("variant", "secondary")
        stats_label.setFixedWidth(120)
        stats_layout.addWidget(stats_label)
        
        self._screencap_count_display = QLabel("0 张")
        self._screencap_count_display.setProperty("variant", "primary")
        stats_layout.addWidget(self._screencap_count_display)
        
        stats_layout.addStretch()
        layout.addWidget(stats_frame)
    
    def _on_screencap_changed(self, state: int) -> None:
        """截图记录开关变更"""
        enabled = state == Qt.CheckState.Checked.value
        settings = {
            'dev': {
                'record_screencap': enabled
            }
        }
        self.settings_changed.emit(settings)
    
    def is_screencap_enabled(self) -> bool:
        """是否启用截图记录"""
        return self._screencap_switch.isChecked()
    
    def set_screencap_enabled(self, enabled: bool) -> None:
        """设置截图记录开关状态"""
        self._screencap_switch.setChecked(enabled)
    
    def update_screencap_count(self, count: int) -> None:
        """更新截图计数显示"""
        self._screencap_count_display.setText(f"{count} 张")
