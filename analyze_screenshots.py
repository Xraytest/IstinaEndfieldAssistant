    ...
    if os.path.exists(providers_file):
        ...
        for key, config in providers.items():
            if key.lower() == provider_name.lower() and config.get('enabled', True):
                return {'api_key': config.get('api_key'), 'api_base_url': config.get('api_base_url'), 'model': config.get('model')}
...
def analyze_screenshot(image_path: str, config: dict) -> str:
    ...
    messages = [{'role': 'system', 'content': "你是一个游戏界面分析专家。请分析截图中的界面元素，特别关注：\n1. 当前是什么界面（主界面/邮箱/暂停菜单/好友列表等）\n2. 界面上有哪些按钮和图标\n3. 按钮的位置（用自然语言描述，如'右上角'、'屏幕底部中间'等）\n4. 特别注意是否有菜单按钮、好友按钮、邮箱按钮\n\n请用简洁的中文回答。"}, {'role': 'user', 'content': [{'type': 'image_url', 'image_url': {'url': f'data:image/png;base64,{image_data}'}}, {'type': 'text', 'text': '请分析这张游戏截图，描述界面布局和按钮位置。'}]}]
    api_base_url = config.get('api_base_url', 'https://coding.dashscope.aliyuncs.com/v1')
    model = config.get('model', 'kimi-k2.5')
    response = requests.post(f'{api_base_url}/chat/completions', headers={'Authorization': f"Bearer {config['api_key']}", 'Content-Type': 'application/json'}, json={'model': model, 'messages': messages, 'max_tokens': 1000}, timeout=60)
